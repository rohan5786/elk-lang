...
static bool end() { return *lx.cur == '\0'; }
...
static bool next_same(char wanted) {
  ...
  if (end() || *lx.cur != wanted) return false;
  lx.cur++;
  return true;
}

static char peek() { return *lx.cur; }
...
static void whitespace() {
  while (1) {
    ...
    switch (c) {
      ...
      case '/':
        if (peek_next() == '/') {
          while (peek() != '\n' && !end()) next_char();  // single line cmnt
        } else
          return;
      default:
...
Token scan_token() {
  ...
  switch (c) {
    ...
    case '(':
      return make_token(LEX_LEFT_PAREN);
    case ')':
      return make_token(LEX_RIGHT_PAREN);
    case '{':
    ...
    case ',':
      return make_token(LEX_COMMA);
    case '.':
      return make_token(LEX_DOT);
    case '-':
      return make_token(LEX_MINUS);
    case '+':
      return make_token(LEX_PLUS);
    case '/':
      return make_token(LEX_SLASH);
    case '*':
      return make_token(LEX_STAR);
    case '!':
      return make_token(next_same('=') ? LEX_BANG_EQUAL : LEX_BANG);
    case '=':
      return make_token(next_same('=') ? LEX_EQUAL_EQUAL : LEX_EQUAL);
    case '<':
      return make_token(next_same('=') ? LEX_LESS_EQUAL : LEX_LESS);
    case '>':
      return make_token(next_same('=') ? LEX_GREATER_EQUAL : LEX_GREATER);
    case '"':
      return string();
    case '&':
      return make_token(next_same('&') ? LEX_AND_AND : LEX_AND);
    case '|':
      return make_token(next_same('|') ? LEX_OR_OR : LEX_OR);
    case '^':
      return make_token(LEX_XOR);
    case '%':
